 ...
 *******************************************************************************/
globalProxyManagementApp.factory('ProxyManagementDateErrorManager', ['globalProxyMgmtDateService', 'notificationCenterService', '$filter',
    function (globalProxyMgmtDateService, notificationCenterService, $filter) {
        var ERRORS = [
                "START_EMPTY_FIELD",
                "STOP_EMPTY_FIELD",
                "START_INVALID_DATE",
            ...
            getDateEmptyErrors = function (startDate, stopDate) {
                var errors = [];
                if (globalProxyMgmtDateService.dateFieldIsEmpty(startDate)) {
                    errors.push(ERRORS[0])
                }
                if (globalProxyMgmtDateService.dateFieldIsEmpty(stopDate)) {
            ...
            getInvalidDateFormatErrors = function (startDate, stopDate) {
                var errors = [];
                if (globalProxyMgmtDateService.dateFormatIsInvalid(startDate)) {
                    errors.push(ERRORS[2])
                }
                if (globalProxyMgmtDateService.dateFormatIsInvalid(stopDate)) {
            ...
            return {
                ...
                setErrorMessages: function (isSubmit) {
                    ...
                    if (this.currentErrors.length === 0) {
                        if (globalProxyMgmtDateService.stopDateIsBeforeStartDate(this.startDate, this.stopDate)) {
