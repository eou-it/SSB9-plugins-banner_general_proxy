...
class ProxyFinAidService {
    ...
    def checkIfFinaidInstalled() {

        boolean isFinaidInstalled
        def session = RequestContextHolder?.currentRequestAttributes()?.request?.session

        if (session?.getAttribute("isFinAidInstalled") != null) {
            isFinaidInstalled = session.getAttribute("isFinaidInstalled")
        }else {
            isFinaidInstalled = InstitutionalDescription.fetchByKey()?.finaidInstalled
            session.setAttribute("isFinaidInstalled",isFinaidInstalled)
        }
        return isFinaidInstalled
    }
}
